<template>
    ...
    <div class="summery" v-if="!loginstore.isloading">
        <div>
            ...
            <span>{{ attendence?.summary?.P }}--{{ attendence?.summary?.P / attendence?.summary?.total * 100 }}%</span>
        </div>
        <div>
            <h4 class="A">Absence</h4>
            <span>{{ attendence?.summary?.A }}--{{ attendence?.summary?.A / attendence?.summary?.total * 100 }}%</span>
        </div>
        <div>
            <h4 class="L">Leaves</h4>
            <span>{{ attendence?.summary?.L }} --{{ attendence?.summary?.L / attendence?.summary?.total * 100 }}%</span>
        </div>
    </div>
    <div v-else>
        <div class="card">
    ...
    <div class="calender" v-if="!loginstore.isloading">
        <div v-for="(day, index) in days"
            :class="{ 'success': getStatus(day) == 'P', 'danger': getStatus(day) == 'A', 'warning': getStatus(day) == 'L', }">
            <span>{{ index + 1 }}</span>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        buildDays() {
            let daysInMonth = moment(this._date).daysInMonth();
            for (let i = 1; i <= daysInMonth; i++) {
                this.days.push(moment(this._date).set('date', i).format('YYYY-MM-DD'));
            }

        },
        getStatus(day) {
            if (!this.attendence?.records || !this.attendence?.records[day]) return '';
